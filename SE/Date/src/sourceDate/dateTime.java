...
public class dateTime {
    ...
    public dateTime addOrSubDays(int amount)
    {
        this.amount = amount;
        return addOrSubDays(this.day + this.amount, this.month, this.year);
    }

    private dateTime addOrSubDays(int days, int month, int year)
    {
        if (days > 0 && days <= getNoOfDaysInMonth(month, year))
        {
            return new dateTime(days, month, year);
        }
        else if (days <= 0)
        {
            month = month - 1;
...
//
//        // Testing
//        // Get the current local time :D
//        Calendar cal = Calendar.getInstance();
////        cal.add(Calendar.DAY_OF_MONTH, amount);
//        System.out.println("The current day is: " + cal.getTime());
//    }
